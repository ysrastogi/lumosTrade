...
from src.agents.athena_workspace.tools.market_summarizer import MarketSummarizer
...
class AthenaAgent:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            agent_id: Unique identifier for this Athena agent instance
            use_memory: Whether to use the memory system integration
        ...
        if self.use_memory and self.memory_manager:
            try:
                ...
                if success:
                    ...
                    logger.warning(f"Failed to initialize memory system for agent {self.agent_id}")
    ...
    async def observe(self, symbol: str, interval: int = 60, count: int = 100) -> Dict:
        ...
        if self.use_memory and self.memory_manager:
            try:
                # Store market observation
                await self.memory_manager.store_observation(symbol, context)
                
                # Store detected patterns as separate memories
                if patterns:
                    await self.memory_manager.store_analysis(
                        symbol=symbol,
                        analysis_type="pattern_detection",
                        analysis_data=patterns
                    )
                
                # Store regime detection as separate memory
                if regime:
                    await self.memory_manager.store_analysis(
                        symbol=symbol,
                        analysis_type="regime_detection",
                        analysis_data=regime
                    )
                ...
                logger.debug(f"Stored {symbol} observation and analysis in memory system")
            except Exception as e:
                logger.error(f"Error storing observation in memory: {str(e)}")
    ...
    async def get_recent_history(self, symbol: Optional[str] = None, limit: int = 10) -> List[Dict]:
        """
        Get recent observations from memory system.
        ...
        if not self.use_memory or not self.memory_manager:
            ...
            if symbol:
                filtered = [c for c in self.context_history if c.get('symbol') == symbol]
        ...
        try:
            # Get recent observations from memory system
            memories = await self.memory_manager.recall_recent_observations(
                symbol=symbol,
                limit=limit
            )
            return memories
        except Exception as e:
            logger.error(f"Error retrieving history from memory: {str(e)}")
            return []
            
    async def get_memory_context(self, symbol: Optional[str] = None) -> Dict[str, Any]:
        """
        Get the full memory context for analysis.
        
        Args:
            symbol: Optional symbol to filter context by
            
        Returns:
            Dict: Memory context including recent memories, messages, events
        """
        if not self.use_memory or not self.memory_manager:
            return {"error": "Memory system not enabled"}
            
        try:
            context = await self.memory_manager.get_context(symbol=symbol)
            return context
        except Exception as e:
